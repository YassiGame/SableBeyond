...
public final class EntityMass {
    ...
    private static volatile boolean livingEntityOnlyPlayerEnabled = true;
    ...
    private static volatile boolean entityFormulaEnabled = true;
    private static volatile boolean entityFormulaInvalid;
    private static volatile @Nullable FormulaManager entityFormula = FormulaManager.compile(DEFAULT_ENTITY_FORMULA);
    private static volatile String entityFormulaExpression = DEFAULT_ENTITY_FORMULA;
    private static volatile @Nullable Double entityFormulaFallbackMass = DEFAULT_BASE_MASS;
    private static volatile boolean itemEntityEnabled = true;
    ...
    private static volatile @Nullable Double itemEntityFallbackMass = DEFAULT_ITEM_ENTITY_FALLBACK_MASS;

    public record MassResolution(double mass, MassSource source) {
    }

    public static void applyConfig(final @Nullable EntityMassConfig config) {
        final EntityMassConfig effective = config == null ? EntityMassConfig.defaults() : config;
        ...
        applyLivingEntityConfig(effective.livingEntity());
        applyItemEntityConfig(effective.itemEntity());
    }
    ...
    public static boolean isLivingEntityOnlyPlayerMassEnabled() {
        return globalEnabled && livingEntityEnabled && livingEntityOnlyPlayerEnabled;
    }

    public static boolean isItemEntityMassEnabled() {
        return globalEnabled && itemEntityEnabled;
    }
    ...
    private static void applyLivingEntityConfig(final EntityMassConfig.LivingEntityConfig config) {
        ENTITY_OVERRIDES.clear();

        livingEntityEnabled = config.enabled;
        livingEntityOnlyPlayerEnabled = config.only_player;
        baseMass = sanitizeMass(config.base_mass, DEFAULT_BASE_MASS);
        volumeMultiplier = sanitizeMass(config.volume_multiplier, DEFAULT_VOLUME_MULTIPLIER);
        applyLivingEntityFormulaConfig(config.formula);

        if (config.entities == null) {
            return;
        }

        for (final Map.Entry<String, Double> entry : config.entities.entrySet()) {
    ...
    private static void applyLivingEntityFormulaConfig(final @Nullable EntityMassConfig.LivingFormulaConfig config) {
        final EntityMassConfig.LivingFormulaConfig defaults = EntityMassConfig.LivingFormulaConfig.defaults();
        final EntityMassConfig.LivingFormulaConfig effective = config == null ? defaults : config;
        ...
        entityFormula = null;
        entityFormulaExpression = effective.expression == null ? defaults.expression : effective.expression;
    ...
    private static void applyItemEntityConfig(final EntityMassConfig.ItemEntityConfig config) {
        ITEM_OVERRIDES.clear();

        final EntityMassConfig.ItemFormulaConfig formulaConfig = config.formula == null
                ? EntityMassConfig.ItemFormulaConfig.defaults()
                : config.formula;

        itemEntityEnabled = config.enabled;
        itemEntityFormulaEnabled = formulaConfig.enabled;
        itemEntityFallbackMass = formulaConfig.fallback_mass == null ? null : sanitizeMass(formulaConfig.fallback_mass, baseMass);
        itemEntityFormulaInvalid = false;
        itemEntityFormula = null;
        itemEntityFormulaExpression = formulaConfig.expression == null
                ? EntityMassConfig.ItemFormulaConfig.defaults().expression
                : formulaConfig.expression;

        if (config.items != null) {
            for (final Map.Entry<String, Double> entry : config.items.entrySet()) {
                final ResourceLocation id = ResourceLocation.tryParse(entry.getKey());
                if (id == null) {
                    continue;
                }
        ...
        final String formula = itemEntityFormulaExpression;
        if (formula.isBlank()) {
